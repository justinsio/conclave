...
async def test_divergence_fewer_than_3_proceeds():
    drafts = [_draft("answer A"), _draft("answer B")]
    result = await check_divergence(drafts, post_is_complex=True)
    assert result.action == "proceed"


async def test_divergence_normal_spread_proceeds():
    drafts = [
    ...
    result = await check_divergence(drafts, post_is_complex=False)
    assert result.action == "proceed"


async def test_divergence_all_high_confidence_complex_triggers_hold():
    drafts = [
...
async def test_divergence_all_high_confidence_not_complex_proceeds():
    drafts = [
        _draft("bloom filter approach", 0.91, "a1"),
        _draft("bloom filter approach similar", 0.90, "a2"),
        _draft("bloom filter approach also", 0.92, "a3"),
...
async def test_divergence_outlier_flagged():
    ...
    drafts = [
        ...
        # Completely different topic — cosine similarity < 0.30 vs others
...
async def test_divergence_no_outlier_similar_drafts():
    # All three drafts share enough tokens that cosine stays above 0.30
    drafts = [
