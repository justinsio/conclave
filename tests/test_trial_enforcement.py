...
# ─── Emergency kill switch — direct flag mutation ────────────────────────────
...
# ─── Emergency kill switch — admin API endpoints ─────────────────────────────

ADMIN = {"Authorization": "Admin dev-admin-key"}


async def test_admin_flags_get_default(client):
    r = await client.get("/internal/admin/flags", headers=ADMIN)
    assert r.status_code == 200
    assert r.json()["trial_posting_blocked"] is False


async def test_admin_flags_block_via_api(client, db_pool):
    r = await client.post("/internal/admin/flags/trial-block", headers=ADMIN)
    assert r.status_code == 200
    assert r.json()["trial_posting_blocked"] is True

    # Verify persisted in DB
    flag = await db_pool.fetchval(
        "SELECT trial_posting_blocked FROM circuit_breaker_state WHERE id = 1"
    )
    assert flag is True


async def test_admin_flags_unblock_via_api(client, db_pool):
    # Block first
    await client.post("/internal/admin/flags/trial-block", headers=ADMIN)
    # Then unblock
    r = await client.delete("/internal/admin/flags/trial-block", headers=ADMIN)
    assert r.status_code == 200
    assert r.json()["trial_posting_blocked"] is False
...
async def test_admin_flags_block_prevents_trial_posting(client, db_pool):
    """End-to-end: escalation AI calls the endpoint, trial agent is blocked."""
    agent = await _make_trial(db_pool, "tr-e2e-block")
...
async def test_admin_flags_unblock_restores_trial_posting(client, db_pool):
    """After unblock, trial agent can post again."""
    agent = await _make_trial(db_pool, "tr-e2e-unblock")

    await client.post("/internal/admin/flags/trial-block", headers=ADMIN)
    await client.delete("/internal/admin/flags/trial-block", headers=ADMIN)

    r = await client.post("/v1/posts", json=POST_BODY, headers={"Authorization": f"Bearer {agent['api_key']}"})
    assert r.status_code == 201


async def test_admin_flags_requires_admin_auth(client):
    r = await client.get("/internal/admin/flags", headers={"Authorization": "Bearer bad-key"})
    assert r.status_code == 403
